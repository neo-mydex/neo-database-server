CREATE TABLE IF NOT EXISTS ai_user_profiles (
  ...
  analyse_count    INT           NOT NULL DEFAULT 0 CHECK (analyse_count >= 0),
  companion_days   INT           NOT NULL DEFAULT 0 CHECK (companion_days >= 0),
  last_active_date DATE
);

COMMENT ON TABLE ai_user_profiles IS 'AI 用户画像：4个维度（1-10分）+ 对应的猫角色和描述 + 注册时间 + 各类计数';
...
COMMENT ON COLUMN ai_user_profiles.registered_at IS '用户注册时间';
COMMENT ON COLUMN ai_user_profiles.trade_count IS '用户在本平台用AI参与的成功交易次数';
COMMENT ON COLUMN ai_user_profiles.chat_count IS '用户发起AI对话的次数';
COMMENT ON COLUMN ai_user_profiles.analyse_count IS '用户发起AI分析的次数';
COMMENT ON COLUMN ai_user_profiles.companion_days IS '用户连续打卡的天数（陪伴天数）';
COMMENT ON COLUMN ai_user_profiles.last_active_date IS '最后一次打卡的日期，用于打卡去重';
